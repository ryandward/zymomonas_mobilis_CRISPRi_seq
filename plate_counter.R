...
zmo_design <-
	zmo_design[
		experiment == "plate" & verbose != "glycerol"]
...
title("MDS: Guide Count by Aerobicity (Plate)")
...
plate_results_LFC <- data.table::dcast(
	melted_results,
	locus_tag + gene_name + type + spacer ~ condition,
	value.var = "LFC")

plate_results_FDR <- data.table::dcast(
	melted_results, 
	locus_tag + gene_name + type + spacer ~ condition,
	value.var = "FDR")
...
plate_results_median_LFC <- data.table::dcast(
	median_melted_results %>% filter(type != "mismatch"),
	locus_tag + gene_name + type ~ condition,
	value.var = "medLFC")

plate_results_median_FDR <- data.table::dcast(
	median_melted_results %>% filter(type != "mismatch"),
	locus_tag + gene_name + type ~ condition,
	value.var = "FDR")

fwrite(plate_results_median_LFC, "plate_results_median_LFC.tsv", sep = "\t")
fwrite(plate_results_median_FDR, "plate_results_median_FDR.tsv", sep = "\t")
