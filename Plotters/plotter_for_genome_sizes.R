...
# This code filters a binary matrix of gene presence/absence in Alphaproteobacteria to identify
# unique genes in individual organisms. The final result is a vector of indices for rows with only
# one "1" value, indicating the presence of unique genes in a single Alphaproteobacteria.
...
# This code creates a data table that summarizes the presence of genes in a set of Alphaproteobacteria. 
# It loops through a vector of indices (uniqueome_indices), which represent unique combinations of gene presence/absence in the Alphaproteobacteria. 
# For each index, the code uses the "extract_comb" and "melt" functions to convert the binary matrix of gene presence/absence into a more usable format. 
# The resulting data table is filtered to include only those genes that are present in the current Alphaproteobacteria. 
# The filtered data table is then added to the final "genes_per_genome" data table, which will contain information about the presence of all genes in all Alphaproteobacteria.
...
orthos_regroup <- orthos %>% 
	...
	mutate(species = paste(Genus, Species)) %>% data.table
...
alpha_summary %>% ggplot(aes(fill = set, y = `gene count`, x = species)) + geom_bar(position = "dodge", stat = "identity") +
	doc_theme +
	theme(axis.text.x = element_text(angle = 45, hjust = 1))

alpha_summary %>% group_by(species) %>% mutate(`gene count` = `gene count`/`gene count`[set == "all_genes"]) %>%
	filter(set != "all_genes") %>%
	rename("genome proportion" = "gene count") %>%
	ggplot(aes(fill = set, y = `genome proportion`, x = species)) + geom_bar(position = "dodge", stat = "identity") +
	doc_theme +
	theme(axis.text.x = element_text(angle = 45, hjust = 1))
